...
public class BinarySearchTree<T extends Comparable<? super T>> implements Iterable<T> {
    ...
    public boolean contains(T x) {
        Entry<T> temp = get(x, root); // method to get the value
	    return temp!=null;  // checking if the returned value is null if null then tree doesn't contain x
    }
    ...
    /** Helper method to find an element in the tree */
    Entry<T> get(T x, Entry<T> ent){
        if(ent==null){
            return null;
        }
        int cmp = x.compareTo(ent.element); // comparing the given element with the entry value
        if(cmp == 0) return ent;
        else if(cmp<0) return  get(x, ent.left); // if compared value is less than 0 traverse left subtree
        else return get(x, ent.right); // else traverse right sub tree
    }
    ...
    Entry add(T x, Entry<T> ent){
        ...
        int cmp = x.compareTo(ent.element);  // comparing the given element with the entry value
        if(cmp<0) ent.left = add(x, ent.left);  // if compared value is less than 0 traverse left subtree
        else if(cmp>0) ent.right = add(x, ent.right); // else traverse right sub tree
        else{
    ...
    /** Helper method to remove an element in the tree */
    ...
    /** Helper method to use in remove method  */
